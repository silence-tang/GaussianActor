...
def process(opt: Options, path):
    ...
    with open(path, 'rb') as f:
        mv_image = np.load(f)
        load_rays_o = np.load(f)
        load_rays_d = np.load(f)

    rays_embeddings = []
    for i in range(load_rays_o.shape[0]):
        rays_o = np.tile(load_rays_o[i][np.newaxis, np.newaxis, :], (256, 256, 1))
        rays_d = load_rays_d[i]
