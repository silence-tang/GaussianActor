...
class ZJUMOCAPDataset(Dataset):
    def __init__(self, dataset_folder,
        ...
        self.sample_views = sample_views # 8
        ...
        if isinstance(view_img_size, numbers.Number):
            self.view_img_size = (int(view_img_size), int(view_img_size))
        else:
        ...
        self.train_cur_views = train_cur_views
        self.test_cur_cam_names = test_cur_views

        self.homo_2d = self.init_grid_homo_2d(img_size[0], img_size[1])
        self.view_homo_2d = self.init_grid_homo_2d(view_img_size[0], view_img_size[1])

        # Get all data
        self.data = []
        self.data_by_frame = {}
        # self.test_cur_cam_sample_array = {}
        ...
        for subject in subjects:
            ...
            if self.mode == 'test':
                # for cam_name in self.test_cur_cam_names:
                #     sample_img_file = glob.glob(os.path.join(cam_dir, '*.jpg'))[0]
                #     sample_array = cv2.cvtColor(cv2.imread(sample_img_file), cv2.COLOR_BGR2RGB)
                #     self.test_cur_cam_sample_array[cam_name] = np.ones_like(sample_array)

                for cam_idx, cam_name in enumerate(self.test_cur_cam_names):
                    cam_dir = os.path.join(subject_dir, cam_name)
                    ...
                    for d_idx, (f_idx, img_file, mask_file, model_file) in enumerate(zip(frames, img_files, mask_files, model_files)):
                        ...
                        frame_data_list = self.data_by_frame_test_cam.get(f_idx)
                        if frame_data_list is not None:
                            frame_data_list.append(data_item)
                        else:
                            self.data_by_frame_test_cam[f_idx] = [data_item]
    ...
    def get_single_item(
        ...
        if is_view:
            img_size = self.view_img_size
            homo_2d = self.view_homo_2d
        else:
        ...
        if sample_rays is not None and self.sample_rays:
            ...
            if sample_rays == 'all':
                ...
                data.update({
                    'sample_pixels': sampled_pixels,
                    'sample_uv': sampled_uv,
                    'sample_ray_dirs': sampled_rays,
                    'sample_ray_dirs_cam': sampled_rays_cam,
                    'sample_body_bounds_intersections': sampled_bounds_intersections,
                    'sample_image_mask': image_mask,
                })
            
            elif sample_rays == 'default':
                ...
                bg_sample_mask = mask_erode_crop == 0
                y_inds_bbox, x_inds_bbox = y_inds, x_inds
                ...
                fg_uv = np.dot(homo_2d.copy()[y_inds, x_inds].reshape([-1, 3]), K_inv.T)
                fg_alpha = np.ones([fg_pixels.shape[0], 1], dtype=np.float32)
                ...
                bg_uv = np.dot(homo_2d.copy()[y_inds, x_inds].reshape([-1, 3]), K_inv.T)
                bg_alpha = np.zeros([bg_pixels.shape[0], 1], dtype=np.float32)

                sampled_pixels = np.concatenate([fg_pixels, bg_pixels], axis=0)
                sampled_uv = np.concatenate([fg_uv, bg_uv], axis=0)
                sampled_rays_cam = self.normalize_vectors(sampled_uv)
                sampled_rays = self.get_camera_rays(R, sampled_uv)
                sampled_alpha = np.concatenate([fg_alpha, bg_alpha], axis=0)
                ...
                sampled_alpha = sampled_alpha[valid_inds, ...]
    ...
    def __getitem__(self, idx):
        ...
        for item in data_list:
            if item in selected_data or item['cam_name'] in self.train_cur_views:
            ...
            if (self.mode == 'train' and item['cam_name'] in self.train_cur_views) or (self.mode == 'val' and item in selected_data):
            ...
            if self.mode in ['train', 'test', 'val'] and item in selected_data:
                view_item_data = self.get_single_item(
                    image,
                    mask,
                    mask_erode,
                    K,
                    ...
                    data_idx,
                    is_view=True,
                    sample_rays='default' if self.mode == 'train' else 'all',
                    min_xyz=min_xyz,
                    max_xyz=max_xyz,
                )
                view_item_data['cam_name'] = cam_name
                view_data_list.append(view_item_data)
        ...
        if self.mode == 'test':
            ...
            #     cam_trans = np.array(self.cameras[cam_name]['T'], np.float32).ravel()
            #     cam_loc = self.get_camera_location(R, cam_trans)
            #     image = self.test_cur_cam_sample_array[cam_name]
            #     mask = image[..., 0]
            #     mask_erode = mask
            #     current_item_data = self.get_single_item(
            #         image,
            ...
            data_list = self.data_by_frame_test_cam[self.data_by_frame_list[idx]]
